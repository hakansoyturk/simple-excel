...
<body>
        ...
        for ($i = 0; $i <= $defaultSheetRowCount; $i++) {
            ...
            for ($j = 0; $j <= $defaultSheetColumnCount; $j++) {
                ...
                if ($counter <= $defaultSheetColumnCount) {
                    ...
                    $uniqueId = $defaultSheetId . $counter;
                    echo "<td id='$counter' oncontextmenu='rightClick(event,$uniqueId);' oninput='updateCell($defaultSheetId,$counter);'  contenteditable='true'>
               
                    </td>
                    <div id='$uniqueId' class='context-menu' style='display:none'>
                    <ul>
                        <li><a onclick='cutCell($counter)'>Cut</a></li>
                        <li><a onclick='copyCell($counter,$defaultSheetId)'>Copy</a></li>
    ...
    <script>
        ...
        function copyCell(location, sid) {
            $.ajax({
                cache: false,
                type: "POST",
                url: "copycell.php",
        ...
        function drawSheet(sid, row, column, fileName, fid) {
            $.ajax({
                ...
                success: function(locationAndData) {
                    ...
                    //alert(infoArray[0]);
                    process(sid, row, column, fileName, fid, infoArray);
                },
                error: function(xhr, ajaxOptions, thrownError) {
                    alert(xhr.status);
                    alert(thrownError);
        ...
        function process(sid, row, column, fileName, fid, infoArray) {
            $("#list tr").remove();
            ...
            for (var i = 1; i <= row + 1; i++) {
                ...
                for (var j = 0; j <= column; j++) {
                    ...
                    if (counter <= column) {
                        ...
                        var uniqueId = String(sid) + String(counter);
                        uniqueId = parseInt(uniqueId);
                        // const newDiv = document.createElement("div");
                        // newDiv.id =uniqueId;
                        // newDiv.innerHTML = "<ul><li><a onclick='cutCell("+counter+")'>Cut</a></li><li><a onclick='copyCell("+counter+","+sid+")'>Copy</a></li><li><a onclick='pasteCell("+counter+","+sid+")'>Paste</a></li></ul>";
                        ...
                        tableTd.setAttribute("oncontextmenu", "rightClick(event," + uniqueId + ");");
                        tableTd.id = counter;

                        //  document.getElementById(counter).appendChild(newDiv);
                        var yazı = "";
                        for (var ii = 0; ii < (infoArray.length); ii += 2) {
                              
                            if (infoArray[ii] == counter) {
                        ...
                        tableTd.innerHTML = ""+yazı + "<div id=" + uniqueId + " class='context-menu' style='display:none'><ul><li><a onclick='cutCell(" + counter + ")'>Cut</a></li><li><a onclick='copyCell(" + counter + "," + sid + ")'>Copy</a></li><li><a onclick='pasteCell(" + counter + "," + sid + ")'>Paste</a></li></ul></div>";

                        //document.getElementById("uniqueId").className='context-menu';
